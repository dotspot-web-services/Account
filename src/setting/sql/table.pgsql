...
-- in review
create table ips(-- new incoming ip resides here before detail is fetched and its deleted 
    ipid BIGINT GENERATED BY DEFAULT AS IDENTITY UNIQUE,
    iprange CIDR PRIMARY KEY,
    ipdt TIMESTAMP with TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP
);
create INDEX ipsdate_idx ON ips USING  brin(ipdt);

create table continent(
    nentid SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    continent_code TEXT,
    continent_name text,
    nentdt TIMESTAMP with TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP
);
create INDEX nentdate_idx ON continent USING  brin(nentdt);

create table country(
    coutid INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nent SMALLINT,
    country_name TEXT UNIQUE,
    country_code2 VARCHAR,
    country_code3 VARCHAR,
    country_capital TEXT,
    calling_code VARCHAR UNIQUE,
    country_tld VARCHAR,
    languages TEXT,
    country_flag TEXT,
    curtime JSONB,
    coudt TIMESTAMP with TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT coutnent_ibfk FOREIGN KEY(nent) REFERENCES continent(nentid)
);
create INDEX curtiz_idx ON country USING  gin(curtime jsonb_path_ops);
create INDEX coudate_idx ON country USING  brin(coudt);

CREATE TABLE city(
    citid INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    couct INT,
    state_prov TEXT,
    district TEXT,
    city TEXT NOT NULL,
    ...
    citdt TIMESTAMP with TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT coutcit_ibfk FOREIGN KEY(couct) REFERENCES country(coutid)
); 
create INDEX citdate_idx ON city USING  brin(citdt);

create table globeip( 
    citglo BIGINT,
    ip BIGINT,
    longitude NUMERIC,
    latitude NUMERIC,
    ...
    ipdt TIMESTAMP with TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT globecit_ibfk FOREIGN KEY(citglo) REFERENCES city(citid),
    CONSTRAINT visitip_ibfk FOREIGN KEY(ip) REFERENCES ips(ipid)
);
create INDEX glodate_idx ON globeip USING  brin(ipdt);
