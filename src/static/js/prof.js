...
forms.forEach(form => {
    ...
    form.addEventListener("submit", (event)=>{
        ...
        switch (myForm.getAttribute("action")) {
            case "/Registry/reg":
                var errors = [
                    checkName(form.elements['fname'].value, '#fname + div.err'), checkContact(form.elements['cont'].value, '#cont + div.err')
                ]
                if(form.elements['dob'] != null){
                    errors.push(checkAge(form.elements['dob'].value, '#dob + div.err'))
                }else{
                    errors.push(checkPwdMatch(form.elements['paswd'].value, form.elements['vpaswd'].value, '#vpaswd + div.err'))
                }
                var i = errors.length;
                while(i--){
                    if(errors[i]===false){
